...
const placeOrder = async (req, res) => {
    ...
    try {
        ...
        for (const storeIdStr in itemsByStore) {
            ...
            const pedidoResult = await client.query(`INSERT INTO pedidos (id_cliente, id_loja, id_endereco_entrega, status, forma_pagamento, valor_total, observacoes)
                 VALUES ($1, $2, $3, 'pendente', $4, $5, $6) RETURNING id`, [clienteId, storeId, id_endereco_entrega, forma_pagamento, valorTotalPedido, observacoes || null]);
            const pedidoId = pedidoResult.rows[0].id;
            createdOrderIds.push(pedidoId);
            // Insere cada item do pedido e atualiza o estoque
            for (const item of storeItems) {
                const productInfoResult = await client.query(`SELECT nome, preco_base FROM produtos WHERE id = $1`, [item.id]);
                ...
                const precoUnitarioResult = await client.query(`SELECT COALESCE(vp.preco_extra, 0) + COALESCE(pp.preco_promocional, p.preco_base) as preco
                     FROM produtos p
                     LEFT JOIN variacoes_produto vp ON vp.id = $1
                     LEFT JOIN LATERAL (
                        SELECT preco_promocional FROM precos_promocao px
                ...
                await client.query(`INSERT INTO itens_pedido (id_pedido, id_produto, id_variacao, nome_produto, tamanho, cor, quantidade, preco_unitario, subtotal)
            ...
            await client.query(`INSERT INTO pagamentos (id_pedido, metodo_pagamento, status_pagamento) VALUES ($1, $2, 'pendente')`, [pedidoId, forma_pagamento]);
        }
