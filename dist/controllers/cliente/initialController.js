...
async function getInitial(req, res) {
    ...
    `;
        ...
        const [lojasResult, categoriasResult, promosResult] = await Promise.all([
            connection_1.pool.query(lojasSql, lojasParams),
            connection_1.pool.query(categoriasSql),
            connection_1.pool.query(promosSql)
        ]);
...
async function listStores(req, res) {
    ...
    `;
        ...
        if (hasPoint) {
            ...
            const orderPrefBase = String(sort).toLowerCase() === 'name'
                ? `nome ASC`
                : String(sort).toLowerCase() === 'orders'
