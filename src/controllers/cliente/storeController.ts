...
export async function getStoreDetails(req: Request, res: Response) {
    try {
        ...
        const storeResult = await pool.query(storeSql, storeParams);
        ...
        const productsByCategory = productsResult.rows.reduce((acc, product: Product) => {
            ...
            return acc;
        }, {} as Record<string, { nome: string; produtos: Product[] }>);
