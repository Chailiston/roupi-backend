...
export const createPaymentPreference = async (req: Request, res: Response) => {
    ...
    if (!items || !Array.isArray(items) || items.length === 0) return res.status(400).json({ message: 'O carrinho está vazio.' });
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            for (const storeIdStr in itemsByStore) {
                ...
                for (const item of storeItems) {
                    ...
                    if (variacao.estoque == null || variacao.estoque < item.quantity) throw new Error(`Estoque insuficiente para: ${variacao.produtos.nome}`);
                ...
                if (loja.frete_gratis_acima_de && subtotalProdutos >= Number(loja.frete_gratis_acima_de)) { valorFrete = 0; }
                if (valorFrete > 0) { preferenceItems.push({ id: `frete_${storeId}`, title: `Frete - ${loja.nome}`, quantity: 1, unit_price: Number(valorFrete.toFixed(2)) }); }
                ...
                for (const item of storeItems) { await tx.variacoes_produto.update({ where: { id: item.variationId }, data: { estoque: { decrement: item.quantity } } }); }
            }
