...
export const placeOrder = async (req: Request, res: Response) => {
    ...
    const { 
        ...
        forma_pagamento, 
        observacoes,
        payment_token, // O token do cartão gerado no frontend
    ...
    try {
        ...
        const customerEmail = (req as any).user?.email; // Pega o email do cliente logado
        ...
        for (const storeIdStr in itemsByStore) {
            ...
            for (const item of storeItems) {
                ...
                if (productResult.rows.length === 0) throw new Error(`Produto com ID ${item.id} não encontrado ou inativo.`);
            ...
            const lojaMpAccountResult = await client.query(
                `SELECT api_key, application_fee_percent FROM loja_payment_settings 
            ...
            const sellerClient = new MercadoPagoConfig({ accessToken: lojistaAccessToken });
            ...
            await client.query(
                `INSERT INTO pagamentos (id_pedido, metodo_pagamento, status_pagamento, mp_payment_id, platform_fee_amount, paid_at) VALUES ($1, $2, $3, $4, $5, NOW())`,
                [pedidoId, forma_pagamento, paymentResult.status, paymentResult.id, valorComissao]
            );
