// src/controllers/clientAddressController.ts
...
export async function createAddress(req: Request, res: Response) {
  ...
  try {
    if (padrao) {
      await pool.query(
        'UPDATE enderecos_cliente SET padrao = FALSE WHERE id_cliente = $1',
        [clientId]
      );
    }
...
export async function deleteAddress(req: Request, res: Response) {
  ...
  try {
    const { rowCount } = await pool.query(
      'DELETE FROM enderecos_cliente WHERE id_cliente = $1 AND id = $2',
      [clientId, addressId]
    );
    if (!rowCount) {
      return res.status(404).json({ error: 'Endereço não encontrado.' });
